...
def main():
    ...
    tmpdir = "%s/positive_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and positived must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "positived"]) is not None:
            print("%sWARNING!%s There is already a positived process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "LITECOIND" not in os.environ:
        os.environ["LITECOIND"] = build_dir + '/src/positived' + exeext
        os.environ["LITECOINCLI"] = build_dir + '/src/positive-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `positive-cli help` (`rpc_interface.txt`).
