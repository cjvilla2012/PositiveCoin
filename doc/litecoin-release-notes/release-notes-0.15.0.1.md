Positive Core version *0.15.0* is now available from:
  ...
  <https://download.positive.org/positive-0.15.0/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/positive-project/positive/issues>

To receive security and update notifications, please subscribe to:

  <https://groups.google.com/forum/#!forum/positive-dev>
...
or `positived`/`positive-qt` (on Linux).
...
Positive Core 0.15.0 contains the following changes to the RPC interface and `positive-cli` for multi-wallet:

* When running Positive Core with a single wallet, there are **no** changes to the RPC interface or `positive-cli`. All RPC calls and `positive-cli` commands continue to work as before.
* When running Positive Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `positive-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Positive Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:9332/wallet/wallet1.dat/`. `positive-cli` commands should be run with a `-rpcwallet` option, for example `positive-cli -rpcwallet=wallet1.dat getbalance`.
