Positive Core version 0.13.2 is now available from:
  ...
  <https://download.positive.org/positive-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a positive
...
This allows running positived without having to do any manual configuration.
...
`positive.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /positiverpc>
        ...
        # optional bypass positived rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< positiverpc:<password>
    </Location>
...
Restarting positived will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in positive.conf
...
positive-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/positive-cli -stdin walletpassphrase
    mysecretcode
    120
...
- `positive-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `positive-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of positives they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of positives being spent is exactly
...
- `positive-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `positive-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
