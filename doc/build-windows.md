...
example /usr/src/positive, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/positive-project/positive.git
    sudo chmod -R a+rw positive
...
way. This will install to `c:\workspace\positive`, for example:

    make install DESTDIR=/mnt/c/workspace/positive
...
causes two of the positive executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the positive code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the positive source code).
